...
| ar-io-verify attested export | `ario.evidence.export/v1` | kernel verdict + inline source `ario.anchor.trace/v1` bundle + embedded operator attestations | `ed25519` wrapper / embedded `rsa-pss-sha256` records | Issuer-composed, offline-verifiable **attested export**: a recomputable kernel verdict over a source anchor-trace bundle plus per-checkpoint RSA-PSS operator attestations, in one signed file. New body, **not a new wrapper.** Authoritative spec: [`evidence-export.md`](evidence-export.md). |
