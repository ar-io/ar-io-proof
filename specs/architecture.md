...
1. **Kernel home + name.** ~~A dedicated `ar-io-envelope` repo (kernel + spec + corpus together) vs a `pkg/` promoted inside `ar-io-agent`. Leaning toward a dedicated repo so no producer "owns" the shared core. Naming: `ario-envelope-{go,py,ts}`.~~ **Resolved at v1.2 coordination (D3 + Names, [`v1.2-coordination/README.md`](https://github.com/ar-io/ar-io-api-guard/blob/main/docs/stack/v1.2-coordination/README.md)):** Python = dedicated **`ar-io-proof`** repo (PyPI `ar-io-proof`, import `ario_proof`; Wave 1 Lane C); TS = **`@ar-io/proof`** (Wave 2 Lane H); Go = **`pkg/proof`** promoted inside `ar-io-agent` (Wave 2 Lane G), satisfying K2 via the public module path — full *outside-the-org* importability lands with Lane G's BSL + per-directory MIT carve-out flip; in the interim K2's anti-drift intent is met by the single in-repo import path. The existing `ar-io-sdk` is orthogonal and unchanged. **Amended 2026-06-11 (TS-kernel-home lane):** the TS kernel's scope is **`@ar.io/proof`** (matching the ecosystem's npm scope `@ar.io/`, *not* `@ar-io/`), and its home moved from the proof-checker app's `packages/proof/` workspace into **`ar-io-proof/ts/`** — this repo, alongside the Python kernel, the family specs, and the authoritative corpus. An app must never own a kernel (the same principle that extracted the Python kernel from mlflow); the proof-checker now consumes `@ar.io/proof` as an ordinary npm dependency. Published from here via `release-ts.yml` (OIDC trusted publishing, provenance on, tags `ts-v*`); first such publish was `0.1.2` (which also carried a Node-ESM packaging fix the workspace had masked). The Go reference remains MIT-carved in `ar-io-agent/pkg/proof`.
