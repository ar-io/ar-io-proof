...
| `data_hash` | Yes | sha256-hex | SHA-256 of the attested transaction's on-chain data. **For an `ario.anchor.trace/v1` source this is `SHA-256(JCS(checkpoint.envelope))`** — the checkpoint tx's data is exactly the uploaded canonical envelope bytes (the same bytes §4.2's on-chain re-fetch compares), so it is offline-recomputable and identical across kernels. A verifier MUST confirm `data_hash` equals that value for the resolved checkpoint (§5 step 6c). |
...
5. **Verdict agreement.** Compare the freshly recomputed verdict to the export's cached `body.kernel_verdict` **over the deterministic, offline-recomputable dimensions only** — per-event `signature_ok` / `payload_bound` / `inclusion_ok` / `content_ok`, per-checkpoint `merkle_root_ok`, and per-attestation `signature_ok` / `operator_address_bound` / `data_hash_bound` / `subject_ref_ok`. A disagreement on those → **exit 1** (recompute-don't-trust, principle 3). The **on-chain per-gateway dimension (§4.2) is explicitly EXCLUDED** from this comparison: the issuer records it online at compose time, but an offline verifier does no re-fetch and legitimately recomputes it as `null`, so comparing it would fail every honest offline export — the verifier's own on-chain outcomes fold in only at step 7. The displayed verdict is always the recomputed one.
